...
- [low] (improvement) `orchestrator/src/nodes/__init__.py` → `_install_post_completion_guard`: The `import json as _json` is a module-level import placed inside the function body to avoid circular imports. Since `json` is stdlib and there is no circular import risk here, moving the import to the module-level import block would be marginally cleaner. Flagging for a future tidy-up pass. **DONE**.
- [low] (debt) `orchestrator/tests/test_graph.py`: 9 tests fail with `ModuleNotFoundError: No module named 'aiosqlite'`. This is documented in `orchestrator/README.md` (Running Tests section) — `aiosqlite` is a `[dev]` extra that must be installed with `pip install -e ".[dev]"`. The test file itself has no `pytest.importorskip` guard, so the failure is noisy rather than a clean skip. Adding `pytest.importorskip("aiosqlite")` at the top of `test_graph.py` would make the gap self-explanatory in CI output. **DONE**.
- [low] (debt) `orchestrator/tests/test_nodes.py::TestSlimPromptContent::test_pm_prompt_has_slim_fields`: Pre-existing failure (assertion on `project_path` string in prompt output). Not caused by this change. **DONE**.
