...
| Agent name mapping | Per-persona YAML (`personas/ledger/src/meta/N-*.yaml`) → naming fields; `_shared.yaml` → `default_version` | `personas/name-mapping.json` (regenerated by `scripts/build-personas.js`; must be regenerated when persona YAML naming fields change; checked into Git — staleness is visible in diffs); consumed by `mcp-server/…/constants.ts` → `AGENT_NAMES` at startup |
...
| Read this workspace's own generated strategic-vision mirror | [.ledger/strategic-vision.md](../../../.ledger/strategic-vision.md) — generated by `ai-insights ledger sync`, never hand-edited; search tools skip hidden directories by default, so this routing line is the reliable way in |
