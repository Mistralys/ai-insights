...
> **Cross-reference: GUI Usage Scenarios (companion initiative — `docs/agents/projects/gui-usage-scenarios.md`).** That project's `usage-scenarios.md` is concrete, decidable, step-by-step end-user walkthroughs — exactly the shape of input this Verifier needs. But per the Critical Design Constraint above, that document as currently designed is **generated from the finished `plan.md`** and is therefore a plan-derived artifact: feeding it to the AV directly would leak the plan's own interpretive frame back into the one check meant to be independent of it. The legitimate connection is upstream, not downstream: if a requester (or the Planner, at ratification time — see Operational Dependency 1) authors or captures usage scenarios **before or during planning, at the goal level**, those scenarios belong in the `request.md` bundle as requester-authored attachments, not as a post-plan artifact read by the AV mid-pipeline. Resolving whether/how to shift scenario capture earlier for AV-eligible input is deferred until Acceptance Verifier implementation is planned.
