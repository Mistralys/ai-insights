...
| `planner-research-brief-template.md` | Agent 1, Planner (Standalone) | `## Strategic Context` is unconditional (ledger-declared-project plan, WP-008); only its source sentence is gated on `{{#if has_mcp}}` (the live `ledger_get_repository_context` MCP tool vs. the `.ledger/strategic-vision.md` mirror file). Includes `{{> strategic-vision-mirror}}` immediately after the fenced template, outside the code fence |
...
| `strategic-vision-mirror.md` | Agent 1, Planner (Standalone) | *(none)* — describes the opt-in `.ledger/strategic-vision.md` mirror (generated, read-only, never hand-edited), how to read its `generated-at`/`vision-hash` header fields, and that `.repositories.json` remains the source of truth. The MCP-specific divergence note (prefer the live `ledger_get_repository_context` tool over the mirror when both are available) is gated behind `{{#if has_mcp}}`. Added by the ledger-declared-project plan (WP-004) as infrastructure; wired into both Planners' `## Strategic Context` section by WP-008 via `planner-research-brief-template.md`'s unconditional inclusion. |
