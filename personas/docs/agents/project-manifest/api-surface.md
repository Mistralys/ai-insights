...
| `{{cc_name}}` | persona `cc_file_name` (`.md` stripped) | Kebab-case Claude Code slash-command identifier: `N-role` (e.g. `3-developer`, `2-project-manager`) |
| `{{cc_description}}` | `_shared.yaml` → `roster[]` `title` + `short` | Human-readable description for Claude Code's auto-delegation display |
| `{{cc_tools_json}}` | `persona.cc_tools` → fallback `_shared.yaml` → `default_cc_tools[]` | YAML flow sequence using `serializeTools()`: `['Bash', 'Read', ...]` |
...
Note: The `name` field is **not quoted** in the CC frontmatter — `cc_name` values are safe kebab slugs derived from `cc_file_name` (`.md` stripped, e.g. `3-developer`). The `vs_file_name` field is omitted; Claude Code uses `cc_file_name` from the YAML metadata as the output filename.
