...
A review entry — mode `Consolidate` — replaces **Changes** with `none — review only` and adds a **Findings** line giving the severity counts split by origin — how many the pass originated, how many it confirmed from an external report — and naming the headline findings in full. The report they came from is deleted once its fixes land, so the entry is the only lasting record of what the review saw.
