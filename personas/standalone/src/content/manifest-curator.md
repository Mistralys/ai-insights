...
**Declared projects.** A `.ledger/settings.json` at the project root marks it as declared to the central ledger, and its `outputs` map may enable a generated strategic-vision mirror (default path `.ledger/strategic-vision.md`; the declaration can override the path). When a declared project has that output enabled, `README.md`'s index carries a routing line pointing at the mirror's actual configured path. In Update mode, add or correct that line as part of the pass. In Audit mode, a missing or wrong-path routing line is a finding — Medium severity, since the mirror stays reachable by direct navigation even when the manifest fails to route to it.
...
| Routing `README.md` to a declared project's generated strategic-vision mirror | Everything under `.ledger/**` — generated, read-only; regenerated by `ai-insights ledger sync`, never authored or corrected here |
