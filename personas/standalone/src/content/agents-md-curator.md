...
| Routing `AGENTS.md` to a declared project's generated strategic-vision mirror | Everything under `.ledger/**` — generated, read-only; regenerated by `ai-insights ledger sync`, never authored or corrected here |
...
- Never write inside `.ledger/**`, in any mode. Where the project root carries a `.ledger/settings.json` declaring a strategic-vision mirror output, `AGENTS.md` carries a routing line to the mirror's actual configured path (default `.ledger/strategic-vision.md`) — add or correct that line the same way any other routing entry is maintained, but treat the mirror itself as read-only, regenerated by `ai-insights ledger sync`.
