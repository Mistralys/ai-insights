...
- **Counts Are a Maintenance Liability:** A description that stays true outlives a number that goes stale — "12 helper classes", "236 tests across 15 files". Any reader, human or agent, can query the current count on demand. A count earns its place only when it carries analytical value that inspection cannot supply, such as a threshold or a trend comparison.
