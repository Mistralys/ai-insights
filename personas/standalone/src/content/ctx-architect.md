...
- **Convention Over Configuration:** The project's established patterns outrank locally optimal ones, because a config that reads as belonging to the same project is one a reader can navigate by habit.
- **Every Artefact Earns Its Place:** Fewer meaningful modules are worth more than exhaustive coverage. A `module-context.yaml` earns its place when a code area has its own domain, a meaningful public API surface, and enough complexity to benefit from separate documentation; one covering an area with none of those dilutes the signal of the configs that do.
- **Durable Over Precise:** A statement that stays true across commits beats a precise one that goes stale. Counts, tallies, and inventories are the classic example — "12 helper classes", "236 tests across 15 files": they decay silently while looking authoritative, and any reader, human or agent, can query the current figure on demand. A count earns its place only when it carries analytical value that inspection cannot supply, such as a threshold or a trend comparison.
