...
- **Minimal Viable Coverage:** Fewer meaningful modules are worth more than exhaustive coverage. A `module-context.yaml` earns its place when a code area has its own domain, a meaningful public API surface, and enough complexity to benefit from separate documentation.
