...
| Add tool-specific or IDE-specific behaviour | This `AGENTS.md` → Tool-Agnostic Policy; affected sub-project's `constraints.md` |
| Add a new `AGENTS.md` anywhere in the workspace | Add its one-line `CLAUDE.md` companion beside it; add both paths to `scripts/lib/health-checks.js` → `claude-md-companion` check's hardcoded path list |
...
> **`CLAUDE.md` is not generated.** Unlike everything above, `CLAUDE.md` (and `mcp-server/CLAUDE.md`)
> is a committed source file containing exactly the single line `@AGENTS.md` — a Claude Code import
> directive, not a copy. `ctx-generate` never writes it. `AGENTS.md` is therefore the sole authority
> for both files: a conflict between the two is never resolved in `CLAUDE.md`'s favour, because
> `CLAUDE.md` carries no content of its own to conflict with. See the Tool-Agnostic Policy above and
> the Cross-System Dependencies row below for the guard that keeps the two files paired.
...
|------------|----------------|------------------------|
| `AGENTS.md` ↔ `CLAUDE.md` companion | `AGENTS.md` (workspace root) and `mcp-server/AGENTS.md` — each is the sole authority for its scope | Its sibling `CLAUDE.md` (workspace root, `mcp-server/CLAUDE.md`), each containing exactly the single line `@AGENTS.md` and nothing else, per the Tool-Agnostic Policy and `personas/standalone/src/content/agents-md-curator.md`; guarded by the `claude-md-companion` instant-tier check in `scripts/lib/health-checks.js`, which hardcodes these two paths — a third `AGENTS.md` requires updating that check's path list |
...
| Insight mutation ownership | `personas/shared/partials/knowledge-ownership.md` | `mcp-server/src/tools/knowledge.ts` → `ledger_update_insight`, `ledger_delete_insight` (granted only to the Knowledge Curator) and `ledger_add_insight` (granted only to the Knowledge Archiver). Consuming personas: `personas/ledger/src/content/1-planner.md`, `personas/ledger/src/content/9-synthesis.md`, `personas/standalone/src/content/developer.md`. The Planner emits a `## Knowledge Base Reconciliation` plan section (template in `personas/shared/partials/planner-output-template.md`, gated on `has_mcp`); the Synthesis and Standalone Developer personas dispatch `ledger-knowledge-curator` in Targeted Reconciliation mode against it, and both declare it in their YAML `subagents`. `personas/ledger-support/src/content/standalone-archiver.md` carries the matching decline-and-redirect constraint. Granting an insight-write tool to any further persona requires updating this partial and the Curator's mode table. |
...
| `.ledger/settings.json` ↔ registry identity and the output contract | Per-project `.ledger/settings.json` (`ProjectSettingsSchema`, `mcp-server/src/schema/project-declaration.ts`) — `schema_version`, `repository_id` (must match a `RepositoryEntry.id` in some configured store), optional `outputs` record keyed by `OutputIdSchema` (currently the single member `'strategic-vision'`) | `mcp-server/src/storage/project-declaration.ts` → `findProjectRoot()` / `loadProjectDeclaration()` / `resolveOutputPath()` (loads and merges `.ledger/settings.local.json`, which may not carry `repository_id` — a local override cannot redirect a project's declared identity); `mcp-server/src/storage/repository-lookup.ts` → `findEntryInStores()` resolves `repository_id` across every configured store (used by `mcp-server/src/utils/repository-identity.ts` → `resolveRepositoryIdentity()`'s `declared` tier, and by `ai-insights ledger init`'s target resolution); `scripts/lib/ledger-project-core.js` / `scripts/ledger-project.js` → the `init`/`edit`/`sync` CLI verbs read and write this file exclusively (never `settings.local.json` from `edit`); `.githooks/pre-commit` → runs `ai-insights ledger sync --check` as a staleness warning when a declaration is present. Changing `OutputIdSchema`'s membership, `DEFAULT_OUTPUT_PATHS`, or the schema's `.strict()` field set requires updating all of the above plus the mirror contract row below. |
| The generated mirror ↔ `StrategicVisionSchema` and the header/hash contract | `mcp-server/src/schema/repository-registry.ts` → `StrategicVisionSchema` (`short_term`/`mid_term`/`long_term`, each nullable) on a `RepositoryEntry` | `mcp-server/src/outputs/strategic-vision.ts` → `renderStrategicVision()` / `renderStrategicVisionBody()` (pure renderers; the five-line generated-header contract — `MARKER`, `repository-id`, `vision-hash`, `generated-at`, DO-NOT-EDIT — and `visionHash()`, a SHA-256 over the three horizon strings only, deliberately excluding `label`/`folder_names`/`last_modified` so an unrelated registry edit never churns the mirror) and `parseGeneratedHeader()` (the inverse parse, used to decide overwrite/remove safety); `mcp-server/src/outputs/sync.ts` → `syncProjectOutputs()` is the sole writer of the mirror file, comparing `vision-hash` and body (never `generated-at`) to decide `written`/`unchanged`; `mcp-server/src/tools/repository-context.ts` → `ledger_get_repository_context`'s optional `mirror` field (`{ path, generated_at, vision_hash, stale }`) reuses the same `visionHash()`/`parseGeneratedHeader()` read-only, never writing. Changing the header's marker string or line order requires updating both `strategic-vision.ts` (render and parse) and the `mirror` field's staleness comparison in `repository-context.ts` in the same change. |
...
## 🛠️ Tool-Agnostic Policy

**Principle:** Generated artefacts, CLI output, advisories, and documentation shipped to consumer projects must not name or depend on a specific IDE, agent harness, or coding tool. Protection and correctness guarantees belong to mechanisms this project itself owns — file headers, the `.ledger/README.md` contract, git hooks, the sync guards — never to a mechanism that only one tool understands.

### Rules

1. **No tool-specific advisories, hooks, or config snippets in shared surfaces.** CLI output, generated files, and cross-project documentation must express a guarantee in terms this project's own mechanisms enforce (a generated-file header, a git hook, a schema guard), never in terms of a named tool's proprietary configuration (e.g. a specific IDE's settings file, a specific harness's hook system).
2. **A tool's own entry-point file may exist, but only as a pointer.** A file whose sole purpose is to route a specific tool to the canonical document (e.g. `CLAUDE.md`, `.github/copilot-instructions.md`) is permitted, but it carries only a pointer to the neutral canonical document (`AGENTS.md`) — never a copy of it and never content of its own. `CLAUDE.md`'s single-line `@AGENTS.md` import (see "Generated Context Docs" below) is the minimal-coupling form of this rule: one import line, with the entire operating manual staying in `AGENTS.md`.
3. **`personas/` target-specific output is the deliberate exception.** The persona build system's per-target output (`vs-code`, `claude-code`, `deep-agents`) is target-specific by design — that is its purpose, not a violation of this policy. This policy governs shared, cross-tool surfaces (root docs, CLI advisories, generated artefacts), not the personas' own multi-target build.

> **Rationale:** The repository's declared long-term vision states *"Focus on the essentials before agentic tools and IDEs with LLM-independence by design. All integrated tools exist only to support the personas."* A workspace with no stated rule against tool-specific coupling has nothing to fail a future plan against — this is exactly how a harness-specific hook advisory shipped through a full planning pipeline unchallenged. Codifying the rule here converts a one-off deletion into a durable check.
...
| File | Purpose |
|------|---------|  
| `scripts/cli.js` | **Interactive command center + direct CLI** for all workspace operations. Replaces `setup-orchestrator.js` as the user-facing entry point. Includes the `ledger` command group (WP-014, ledger-declared-project plan): `cmdLedger(args)` dispatches `init`/`edit`/`sync` to the shells in `scripts/ledger-project.js`, and — when invoked with no sub-verb from the interactive menu or a bare `ai-insights ledger` on a TTY — resolves the current directory's declaration state via `resolveLedgerDeclaration()` and infers the verb through `chooseDefaultVerb()` (`scripts/lib/ledger-project-core.js`) in `runLedgerInference()`, printing the inferred verb and resolved root before the first prompt. The `ledger` `COMMANDS` entry is menu-visible (`key: 'n'`, `category: 'MCP Server'`) — unlike `store`'s `key: null` — since its audience is expected to reach it via the menu rather than recall the sub-verb. A non-TTY verbless invocation exits 1 naming all three verbs instead of inferring; the interactive path (menu selection, or a bare `ledger`/`ledger init`/`ledger edit` on a TTY with no flags) awaits a keypress via `waitForKey()` before returning so the wizard's summary survives the menu redraw — `sync`, `--check`, and every flag-driven invocation return immediately without reading a key. |
...
| `scripts/launch-agent.js` | Thin CLI entry point for `ai-insights agent`: scans `~/.claude/agents/`, presents a type-to-filter picker (raw-mode interactive, or a numbered `readline` fallback when stdin isn't a TTY), and launches `claude --agent <id>` with the selection, forwarding its exit code. Supports `--filter <term>` and passthrough args after a bare `--`. Invokable via `node scripts/cli.js agent`. |
| `scripts/lib/launch-agent-core.js` | Pure discovery/filtering/reducer logic and both picker I/O shells for `scripts/launch-agent.js`. Exports `discoverAgents(agentsDir)` (role: frontmatter → label, else filename-without-extension), `filterAgents(agents, query)`, `reducePickerInput(state, input, filteredLength)`, `runInteractivePicker(agents)`, and `runNonInteractivePicker(agents, initialQuery, { readlineFactory })`. No `main()`/CLI logic — imported by both `launch-agent.js` and its test file. |
...
| `scripts/install-hooks.js` | One-time setup: sets `git config core.hooksPath .githooks` to activate the pre-commit guard (persona freshness, version sync, ruff lint, presentation `dist/` staleness warning, CTX staleness warning, changelog drift warning, `.ledger/` declaration staleness warning) |
...
| `scripts/backfill-duration.js` | **One-time backfill:** populates `duration_ms` in `.meta.json` for existing projects that already have `synthesis_generated_at` on their root index but predate the enrichment-cache field. `duration_ms = synthesis_generated_at - date_created`, nulled out for zero-duration standalone same-session imports. Supports `--dry-run` and `--verbose`. Idempotent — skips projects that already have a non-null `duration_ms`. Store discovery: `~/.ai-insights/stores.json` → `LEDGER_ROOT` env var. Project-directory discovery is delegated to `scripts/lib/ledger-dirs.js` (never re-implemented locally). Invokable via `node scripts/cli.js backfill-duration`. |
...
| `scripts/lib/ledger-bridge.js` | **Generalized dist-loading bridge for root-level scripts** (ledger-declared-project plan, WP-002). Exports `loadDistModule(relativePath)` — rebuilds `mcp-server/dist/` when stale (same freshness guard `ledger-dirs.js` used to own) and dynamically `import()`s the requested compiled module by its path relative to `mcp-server/dist/`, caching each distinct `relativePath` in a module-level map so repeated calls for the same module return the same cached import. Generalizes the single-purpose loading mechanism `ledger-dirs.js` used to implement inline, so further compiled `mcp-server/dist/` dependencies can be reached without a new near-identical loader file. |
| `scripts/lib/ledger-dirs.js` | **Canonical project-directory discovery for root-level scripts.** Thin re-export of `LedgerStore.listAllProjectDirs()` — as of the ledger-declared-project plan (WP-002), the dist-loading mechanism itself lives in `scripts/lib/ledger-bridge.js` (`loadDistModule()`); this module calls that bridge and re-exports the result as `listAllProjectDirs(storeRoot)`, preserving its original signature and contract. Every root-level script that needs to enumerate ledger project directories (`scripts/backfill-duration.js`, `scripts/import-standalone.js`, `scripts/lib/store-commands.js` → `storeList()`) calls this helper instead of re-implementing flat-vs-namespaced layout detection — that logic is owned exclusively by `LedgerStore.listAllProjectDirs()` in `mcp-server/src/storage/ledger-store.ts`. |
| `scripts/lib/ledger-project-core.js` | **Pure decision core behind the `ai-insights ledger` command group** (`init`, `edit`, and the verbless entry point; ledger-declared-project plan, WP-011). Exports `resolveInitTarget({ cwd, repositoryId, ledgerRoot })` (classifies the `init` target as `matched` / `ambiguous` / `unregistered` against `listEntriesInStores()`, via the bridge, flagging derived-name candidates), `buildInitialSettings({ repositoryId, outputs })` (the `settings.json` object, every known output defaulting to `{ enabled: false }`), `planInitWrites({ projectRoot, settings, force, dryRun })` (the `.ledger/settings.json` + `.ledger/README.md` write set, or a refusal when `settings.json` exists without `force`), `collectAdvisories({ projectRoot })` (the gitignore line, the two doc-routing lines, and the public-repository warning, as structured `{ id, text, applicable }` records — only the gitignore record is ever `applicable: false`), and `chooseDefaultVerb({ cwd, projectRoot, declarationState })` (infers `init`/`edit`/`invalid_declaration` from already-resolved `findProjectRoot()`/`loadProjectDeclaration()` results). Follows the `launch-agent-core.js` core/shell split: no prompting, no filesystem writes, and no `process.exit` — every side effect belongs to the `init`/`edit` shells in `scripts/ledger-project.js` (WP-012). `GITIGNORE_LINE` (`/.ledger/settings.local.json`) is exported as a constant so the wizard shell can append it without re-declaring the string. |
| `scripts/ledger-project.js` | **`init`, `edit`, and `sync` verb shells over `scripts/lib/ledger-project-core.js`** (ledger-declared-project plan, WP-012/WP-013; wired into `scripts/cli.js`'s `ledger` command group by WP-014 — see that row below for the menu/dispatch glue). `init` and `edit` each have a non-interactive flag-driven path (triggered by any CLI flag, or by `stdin` not being a TTY) and an interactive wizard path (TTY, no flags), sharing prompt/confirm helpers built on `askCleanInput()`. `init` resolves the registry entry via `resolveInitTarget()` (by `--repository-id` or derived folder name), writes `.ledger/README.md` and `.ledger/settings.json`, refuses to overwrite without `--force`, and supports `--dry-run`; the wizard path presents registry candidates with the derived-name match preselected, prints the public-repository warning immediately above each output's `Enable …? (y/N)` prompt (default No), and is asserted byte-identical to the equivalent flag-driven run. `edit` resolves the project root via `findProjectRoot()`, prefills wizard prompts from the current declaration, writes `settings.json` only (never `settings.local.json`), rejects `--force`, and errors pointing at `ledger init` for a `not_declared` or `invalid` declaration; disabling an output through `edit` routes through `syncProjectOutputs()`'s removal path, the same one a hand-edited `enabled: false` would take. `sync` (`runSync()`) is the third verb — always non-interactive (no `readAnswer` parameter, no TTY branch, ever), so it is safe to invoke from the pre-commit hook (WP-015) or CI without risk of hanging on a read. It resolves the project root the same way `edit` does (`findProjectRoot()`'s ancestor walk from `cwd`, `.ledger/`-only contract) and errors on `not_declared`/`invalid` with the same message shape (naming the walked directory, pointing at `ledger init`); default mode writes via `syncProjectOutputs({ mode: 'write' })` and exits non-zero when any output reports `blocked`, while `sync --check` runs `syncProjectOutputs({ mode: 'check' })` (no I/O) and exits 1 when any output is stale or missing, 0 when current. All three verbs resolve the project root from `getOriginalCwd()`, not the workspace root. The wizard's run-sync-now offer and `edit`'s disabled-output cleanup call `syncProjectOutputs()` (WP-009) directly through the bridge (`performSync()`) rather than shelling out to the `ledger sync` CLI verb — both that helper and `sync`'s own write path converge on the same `syncProjectOutputs()` choke-point. |
| `scripts/lib/store-commands.js` | Pure-JavaScript implementation of the `store` CLI command group (cross-device ledger sync plan, WP-006); **public command functions** consumed by `scripts/cli.js → cmdStore()`: `storeInit`, `storeAdd`, `storeRemove`, `storeList`, `storeSetDefault`, `storeConflicts`, `storeStatus`, `storeRepoAdd`, `storeRepoMove`, `storeRepoList` — all `async` as of the ledger-declared-project plan (WP-007), joining the already-`async` `storeList`; `cmdStore()`'s dispatch sites all `await` them; **exported for test isolation only** (not public CLI API): `resolveConfigPath`, `expandPath`, `registryPath`, `loadConfig`, `saveConfig` (also `async`, WP-007), `loadRegistry`, `saveRegistry` — these I/O helpers let tests pre-seed config/registry files in temporary directories via the `configPath` override parameter; `saveConfig()` and `storeInit()` additionally accept `_storesDirOverride` for test isolation (overrides the `~/.ai-insights/` directory used for `mkdirSync`); `storeRemove()` sets `default_store: null` when the last store is removed (the resulting invalid config causes `loadStoresConfig()` to return null and fall back to legacy single-store mode on next server load); as of WP-007, `loadRegistry`/`saveRegistry` and `expandPath`/`resolveConfigPath` no longer implement `.repositories.json`/path-resolution I/O locally — they delegate through `scripts/lib/ledger-bridge.js` (`loadDistModule()`, WP-002) to the compiled, lock-protected, schema-validated `mcp-server/src/storage/repository-registry.ts` and `store-registry.ts`; `loadConfig`/`saveConfig` (`stores.json` I/O) deliberately remain local, unlocked read/write rather than delegating to `loadStoresConfig()`/`saveStoresConfig()`, because `StoresConfigSchema` cannot represent the transitional empty-stores/`default_store: null` state `storeRemove()` needs to persist when the last store is removed; file formats are otherwise compatible with the TypeScript `StoresConfigSchema` and `RepositoryRegistrySchema`; cross-platform (uses `path.join`/`path.resolve`, no shell deps). |
...
| Declare this workspace's repository-side output mirror | `ai-insights ledger init` / `ledger edit` / `ledger sync` (root [README.md](README.md) → Ledger Project Declaration); read this workspace's own mirror at [.ledger/strategic-vision.md](.ledger/strategic-vision.md) when declared |
