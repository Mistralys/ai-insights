...
| `title` field (project display title) | `ledger_initialize_project` → `InitializeProjectSchema.title` (tool parameter, `z.string().min(1).max(200).optional()`); `ledger_import_standalone` → `ImportStandaloneSchema.title` (same constraints) | `mcp-server/src/storage/ledger-store.ts` → `MetaCacheUpdates.title?: string` (non-nullable; uses `!== undefined` semantics, not `'key' in cacheUpdates` — no "clear title" use case); `writeProjectMeta()` cacheUpdates handler spreads `title` into `.meta.json` after the existing title-preservation line; `importStandaloneProject()` calls `updateTitle(detail.title)` after `writeRootIndex()` (ordering constraint: `updateTitle()` reads the `.meta.json` that `writeRootIndex()` creates); `LedgerStore.ImportStandaloneDetail.title?: string`; `mcp-server/src/schema/project-meta.ts` → `ProjectMetaSchema.title` (storage, `z.string().optional()`); `mcp-server/gui/api.ts` → `handleListProjects` / `handleGetProject`: `meta.title` takes precedence over slug-derived title-casing when set (same precedence as before — this field was already in the GUI; only the tool-parameter input path is new). Persona agents (Ledger Bootstrapper, Standalone Archiver) should pass a curated `title` alongside `project_summary` when calling these tools — the title should be a human-readable label like `"API: Split GetTenants"` or `"Cross-Platform Plugin Phase 3B"` that no slug pattern-matcher could generate. Changing the field's constraints (min/max) requires updating both persona instructions. |
