...
| Agent name mapping | Per-persona YAML (`personas/ledger/src/meta/N-*.yaml`) → `role`, `number`, `id`, `changelog` (version source — `version` and `last_updated` are auto-derived from its first entry; never add standalone `version:` or `last_updated:` fields), `cc_file_name`, `vs_file_name`, `da_file_name`; `_shared.yaml` → `default_version` (fallback for personas without a `changelog` entry). **Two payloads:** naming fields are always emitted (the MCP server `require()`s this file at startup); the model fields (`model`, `model_slug`, `cc_model`) are emitted **only** when `personas/model-registry/local.json` or `assignments.json` exists — without a local registry those values are machine-dependent and would churn the committed file on every clone. | `personas/name-mapping.json` (regenerated by `scripts/build-personas.js` after every real build; must be regenerated when persona YAML naming fields change; checked into Git — staleness is visible in diffs); consumed by `mcp-server/src/utils/constants.ts` → `AGENT_NAMES` constant at startup |
...
| Sibling package linking | `.dev-links.json` (workspace root, gitignored) | `scripts/dev-link.js` → link/unlink/status logic (writes and removes the marker); `scripts/lib/health-checks.js` → `dev-links-inactive` instant check (reports DEV mode as failing so the CLI status line surfaces it); `scripts/lib/precommit-guards.js` → `devModeInactive()` and `noFileProtocolInLocks()` are the actual DEV-mode block and `file:`-lock enforcement point (resolving `dev-links-inactive` from `HEALTH_CHECKS` by id), with `.githooks/pre-commit` reduced to a thin shell shim that delegates to `scripts/precommit-guards.js`; `scripts/preflight-bootstrap.js` → resolves the same `dev-links-inactive` check by id (hard-throw guard on a missing id) and prints a `[Bootstrap]`-prefixed advisory before any install/rebuild work, without altering its exit code; `docs/references/development.md` → documented DEV/PROD workflow and agent rule. Only `node_modules/` is affected — `package.json` and `package-lock.json` must never be pointed at local paths. |
...
| Pre-commit guards | Node.js `child_process`/`fs` module (`scripts/lib/precommit-guards.js`) behind a minimal shell PATH-resolution shim (`.githooks/pre-commit`) — guard logic itself runs on Windows | `scripts/lib/precommit-guards.js`, `scripts/precommit-guards.js` |
...
The workspace runs in one of two dependency modes: **PROD** (registry-resolved, the default and
the only state valid for commits) and **DEV** (one or both siblings symlinked into `node_modules/`
via `npm link`).

### Agent Rule: Autonomous DEV Mode Activation

When an agent receives a task that requires modifying source code in `ai-persona-builder` or
`cli-menu`, the agent **must** activate DEV mode before making changes:

1. **Check current mode:** Read `.dev-links.json` in the workspace root, or run `node scripts/dev-link.js status`.
2. **Activate if needed:** Run `node scripts/cli.js dev-link --package <name>` to symlink the affected package.
3. **After changes:** Rebuild the sibling (`npm run build` in the sibling directory) and verify propagation (e.g. `node scripts/build-personas.js --check` for persona-builder).
...
The pre-commit hook blocks commits while `.dev-links.json` exists, so forgetting to unlink is a
safe failure. It also rejects `file:` resolved paths added to staged lock files — the same
contamination the `release-check` skill rejects at release time.
...
| `scripts/lib/precommit-guards.js` | **Cross-platform pre-commit guard registry.** Exports `getStagedFiles()`, the declarative `GUARDS` array, `runGuards(guards, stagedFiles)`, and one function per guard (`personaFreshness`, `versionSync`, `devModeInactive`, `noFileProtocolInLocks`, `ruffLint`, `contextStaleness`, `changelogDrift`) plus the pure `diffAddsFileProtocol(diffText)` predicate. Every guard returns `{ id, blocking, passed, messages }` instead of printing or exiting directly, so it runs identically on Windows, macOS, and Linux and is unit-tested in isolation. |
| `scripts/precommit-guards.js` | Thin runner invoked by `.githooks/pre-commit`: computes the staged file list, iterates `GUARDS` via `runGuards()`, and exits 1 on the first blocking failure (0 otherwise). Advisory guards print warnings without affecting the exit code. |
...
| `scripts/dev-link.js` | Link/unlink sibling repos (`ai-persona-builder`, `cli-menu`) for local development via `npm link`. Writes the gitignored `.dev-links.json` marker. Subcommands: `link [--package <name>] [--skip-build]`, `unlink [--package <name>]`, `status`. Skips packages whose sibling directory is absent with a warning. Invokable via `node scripts/cli.js dev-link` / `dev-unlink`. |
