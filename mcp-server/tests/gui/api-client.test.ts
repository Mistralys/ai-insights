...
declare global {
  ...
  var API: {
    getRunLogs: (repo: string, slug: string) => Promise<unknown>;
    getRunLogEntries: (repo: string, slug: string, filename: string, afterLine?: number) => Promise<unknown>;
...
describe('API.getRunLogs', () => {
  it('calls GET /api/projects/{repo}/{slug}/runs', async () => {
    const calls = mockFetch([]);

    await globalThis.API.getRunLogs('my-repo', 'my-slug');

    expect(calls).toHaveLength(1);
    expect(calls[0]!.url).toBe('/api/projects/my-repo/my-slug/runs');
    expect(calls[0]!.opts.method).toBe('GET');
  });

  it('encodes both repo and slug via encodeURIComponent', async () => {
    const calls = mockFetch([]);

    await globalThis.API.getRunLogs('my repo', 'slug with spaces');

    expect(calls[0]!.url).toBe('/api/projects/my%20repo/slug%20with%20spaces/runs');
  });
});
...
describe('API.getRunLogEntries', () => {
  it('calls GET /api/projects/{repo}/{slug}/runs/{filename} without ?after when afterLine is omitted', async () => {
    const calls = mockFetch({ entries: [], totalLines: 0 });

    await globalThis.API.getRunLogEntries('my-repo', 'my-slug', 'file.jsonl');

    expect(calls).toHaveLength(1);
    expect(calls[0]!.url).toBe('/api/projects/my-repo/my-slug/runs/file.jsonl');
  ...
  it('appends ?after={afterLine} when afterLine is provided', async () => {
    ...
    await globalThis.API.getRunLogEntries('my-repo', 'my-slug', '20260225T113355-my-slug.jsonl', 5);

    expect(calls).toHaveLength(1);
    expect(calls[0]!.url).toBe('/api/projects/my-repo/my-slug/runs/20260225T113355-my-slug.jsonl?after=5');
  });
  ...
  it('encodes the filename via encodeURIComponent', async () => {
    ...
    await globalThis.API.getRunLogEntries('my-repo', 'my-slug', 'file name.jsonl');

    expect(calls[0]!.url).toBe('/api/projects/my-repo/my-slug/runs/file%20name.jsonl');
  });

  it('encodes both repo and slug via encodeURIComponent', async () => {
    const calls = mockFetch({ entries: [], totalLines: 0 });

    await globalThis.API.getRunLogEntries('repo/with/slashes', 'slug/with/slashes', 'file.jsonl');

    expect(calls[0]!.url).toBe('/api/projects/repo%2Fwith%2Fslashes/slug%2Fwith%2Fslashes/runs/file.jsonl');
  });
  ...
  it('appends ?after=0 when afterLine is explicitly 0 (valid offset)', async () => {
    ...
    await globalThis.API.getRunLogEntries('my-repo', 'my-slug', 'file.jsonl', 0);
