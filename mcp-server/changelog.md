...
### Changed
- **WP-001:** Enabled `"noUncheckedIndexedAccess": true` in `mcp-server/tsconfig.json`. All string-indexed record lookups are now widened to `T | undefined` at the compiler level. Six type errors surfaced across four files; each was resolved with an explicit narrowing guard (`if (!value)` / `if (value === undefined)`) — no non-null assertions (`!`) used. Modified files: `src/index.ts` (extracted `process.argv[argIdx+1]` into a const with an if-guard), `src/tools/workflow.ts` (destructured `blockedWps[0]` into `firstBlocked` with an undefined guard), `src/utils/agent-registry.ts` (assigned `nameMatch[1]` and `roleMatch[1]` to local consts with undefined checks), `src/utils/wp-id.ts` (assigned `match[1]` to a const with undefined check before `parseInt`). `tsc --noEmit` exits clean; all 251 tests pass.
- **WP-002:** `src/tools/workflow.ts` — removed the manual `export const _internal = { ... }` object (18 entries). Each formerly-private function is now exported directly: `buildHandoffPrompt`, `isStalePipeline`, `extractStalePipelineAction`, `extractReworkAction`, `getDeveloperAction`, `getHandoffNotesForAgent`, `isMostRecentPipelineFail`, `nextAgentFromStatus`, `buildHandoffResponse`, `getProjectManagerHandoff`, `getDeveloperHandoff`, `getQaHandoff`, `getReviewerHandoff`, `getDocumentationHandoff`. Constants `STALE_PIPELINE_HOURS` and `MAX_HANDOFF_DEPTH` are now directly exported. `PIPELINE_AGENT_MAP` and `NEXT_AGENT_MAP` are re-exported from `pipeline-maps.js`. Test files `workflow-handoff.test.ts` and `auto-handoff.test.ts` updated to `import * as _internal from workflow.js` (namespace import); any future export is automatically included with no manual sync required. Note: ESM prohibits circular self-re-export (`export * as _internal` is not possible), so individual named exports with a namespace import in tests is the idiomatic alternative.
- **WP-003:** `scripts/check-known-roles.js` — added a `parseArray(source, pattern, label)` helper centralising array-literal extraction. Both parse blocks (`AGENT_ROLES` from `mcp-server/dist/` and `KNOWN_ROLES` from `sync-personas.js`) now delegate to this helper. Regex patterns updated from `[^\]]+` to `[\s\S]+?` with the `s` (dotAll) flag for correct multiline parsing. Parse failure calls `process.exit(1)` with a labelled error message.
- **WP-004:** `tests/utils/agent-registry.test.ts` — migrated `stderrSpy` from inline `it`-body declarations to `beforeEach`/`afterEach` lifecycle hooks in all five affected `describe` blocks (`AC: files without role:`, `AC: files with role: but no name:`, `AC: non-existent directory`, `AC: strict mode`, `AC: role collision warning`). Removed 9 inline `const stderrSpy = vi.spyOn(...)` declarations and their corresponding `mockRestore()` calls; removed a redundant `stderrSpy2` local. Ensures consistent spy teardown and eliminates spy-leak risk between tests.
- **WP-005:** `src/utils/agent-registry.ts` — added `@param strict` JSDoc tag to `discoverAgents()` documenting its intended use as a CI/validation tool, the `RangeError` throw behaviour when `strict: true`, and the false-default case (emit `stderr` warning, continue). No behaviour change.

## v1.4.2 - Gold Nuggets Housekeeping (2026-02-20)

### Changed
- **WP-001 / GN#1:** `workflow.ts` — removed the local `type AgentRole = typeof AGENT_ROLES[number]` re-derivation (which duplicated the definition in `constants.ts`) and replaced it with `import { AGENT_ROLES, type AgentRole }` from `constants.js`. Single source of truth; the `import type` modifier on the named binding is erased at compile time with no runtime footprint.
- **WP-001 / GN#3:** `agent-registry.ts` — standardised all `stderr` output to the uniform `[agent-registry]` prefix. Two `[discoverAgents]` occurrences (the `RangeError` throw at line ~137 and the collision warning at line ~146) have been updated; every `process.stderr.write()` call in the file now uses the same module tag, making log output grep-stable.
- **WP-001 / GN#4:** `agent-registry.test.ts` — replaced the two identical `.toMatch(/Dev A|Dev Z/)` assertions in the collision-warning describe block with two distinct independent assertions: `.toMatch(/Dev A/)` and `.toMatch(/Dev Z/)`. Each assertion now pinpoints a specific agent name; failure messages are self-describing. A logically redundant `.toSatisfy` guard (always vacuously satisfied by the preceding `.toMatch` pair) was also removed.
- **WP-001 / GN#5:** `workflow.ts` — differentiated the two `buildHandoffResponse` catch-block labels that were both `storage error`. The first is now `storage error (auto-handoff depth update)` (depth-counter write failure) and the second is `storage error (COMPLETE depth reset)` (depth-counter reset on project completion). The outer `[buildHandoffResponse]` prefix is unchanged so existing log grep patterns continue to work.
- **WP-004 / GN#7:** `data-flows.md` — reordered the 13 numbered flow sections from their previous non-sequential arrangement (1–7, 12, 10, 11, 8, 9, 13) to strict ascending numeric order (1–13). The `Data Flow Patterns` appendix remains as the final section. All section content (prose, tables, code blocks) is preserved verbatim; only the sequence of sections was corrected.
