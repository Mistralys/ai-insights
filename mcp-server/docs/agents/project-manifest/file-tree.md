...
│       ├── api-client.js    # API IIFE — all fetch helpers (getProjects, getProject, getWorkPackage, getPlanDocument, deleteProject, archiveProject, unarchiveProject, getConfig, updateConfig, getInsightsStats); buildQueryString(params) helper used by getProjects
...
│       │   ├── project-detail.js  # extractSynopsis, renderPlan, renderSynthesis, renderProjectDetail; PIPELINE_STAGES constant; showResetModal (includes markCompleteMode toggle — Mark All as Complete button force-completes all non-CANCELLED WPs via API.markProjectComplete; cancel override reverts to normal reset mode); archive banner with inline unarchive action
│       │   ├── work-package.js    # renderWorkPackageDetail
