...
└── tests/                       # Test suites
    ...
    │   ├── api-run-metadata.test.ts  # 5 tests for handleGetRunMetadata: HTTP 200 with parsed metadata (AC-1), HTTP 404 when file absent (AC-2), HTTP 404 when project has no plan_path (AC-3), HTTP 400 for unsafe slug (AC-4), file path constructed as path.join(planPath, '.orchestrator-run.json') (AC-5); real temp dirs + LedgerStore fixtures
