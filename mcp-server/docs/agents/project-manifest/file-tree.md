...
│   │   ├── knowledge.ts         # InsightScope ('global'|'repository'), PROJECT_SLUG_REGEX, InsightSchema / Insight (fields: id, scope, repository_name?, origin_plan?, title, content, category, tags, source, created_at, updated_at?, confidence, superseded_by?), KnowledgeStoreSchema / KnowledgeStore — Zod schemas for the knowledge accumulation system (WP-001)
...
└── tests/                       # Test suites
    ...
    │   ├── knowledge-repository-scope.test.ts  # 31 integration tests for repository-scope knowledge functionality across two layers (WP-010): storage layer (17 cases) — repositoryStorePath path generation and reserved-name guard, addInsight with repository scope, readRepositoryStore empty/populated, listInsights unfiltered/scope-filtered/name-filtered, searchInsights with repository_name, updateInsight and deleteInsight with repository scope, moveInsight global→repo/repo→repo/same-name rejection, origin_plan preservation through add+update+move; GUI REST handlers (14 cases) — handleListKnowledge with repository_name, handleUpdateKnowledge with repository scope, handleDeleteKnowledge success and missing-repository_name, handlePromoteKnowledge from repository (success) and from global (rejection), handleMoveKnowledge global→repo/same-repo rejection/missing-target rejection, scope:'project' rejection by all 4 mutating handlers (VALIDATION_ERROR), handleListKnowledge silently ignores scope:'project' (documents safeParse lenient behaviour); real temp dirs + KnowledgeStoreManager — no mocks; follows knowledge-api.test.ts patterns
