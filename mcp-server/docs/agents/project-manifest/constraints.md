...
**Slug casing validation:** `planFolderBasename()` / `validatePlanPath()` only check the `{YYYY-MM-DD}-` date prefix — they accept any casing for the rest of the folder name. The GUI, however, enforces an all-lowercase slug on every `/api/projects/:repo/:slug` route via `assertSafeSlug()` (`SAFE_SLUG_REGEX`, `src/utils/constants.ts`). A folder like `2026-09-22-MS01-my-feature` used to pass `validatePlanPath()`, get written into the ledger, and then fail every GUI detail-page load with `"Invalid repo or slug parameter."` — a mismatch discovered only after the project was already archived. `validateSlugSafety()` in `src/utils/path-validator.ts` closes this gap: `initializeProject()` and `importStandalone()` (the two project-creation entry points) call it immediately after deriving the slug and reject a non-conforming folder name — with a message suggesting the corrected lowercase equivalent — before any ledger write occurs. Read/update call sites (`getProjectStatus`, `updateSynthesis`, pipeline/work-package tools, etc.) must **not** adopt this check — they operate on already-existing projects that may predate the rule.
