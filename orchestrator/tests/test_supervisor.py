...
def make_tool(name: str, return_value: Any) -> MagicMock:
    """Return a mock LangChain Tool that returns *return_value* when invoked."""
    tool = MagicMock()
    tool.name = name
    tool.invoke = MagicMock(return_value=json.dumps(return_value) if not isinstance(return_value, str) else return_value)
    return tool
...
def make_mcp_tools(
    *,
    ...
    def wp_detail_side_effect(kwargs: dict) -> str:
    ...
    detail_tool.invoke = MagicMock(side_effect=wp_detail_side_effect)
...
class TestRouteToPM:
    def test_no_wps_routes_to_pm(self):
...
class TestRouteToDeveloper:
    def test_wp_with_no_pipelines_routes_to_developer(self):
    ...
    def test_implementation_fail_routes_to_developer(self):
    ...
    def test_qa_fail_routes_to_developer(self):
    ...
    def test_code_review_fail_routes_to_developer(self):
...
class TestRouteToQA:
    def test_pass_impl_no_qa_routes_to_qa(self):
...
class TestRouteToReviewer:
    def test_pass_qa_no_review_routes_to_reviewer(self):
...
class TestRouteToDocs:
    def test_pass_review_no_docs_routes_to_docs(self):
...
class TestRouteToSynthesis:
    def test_all_complete_routes_to_synthesis(self):
    ...
    def test_all_pipelines_pass_routes_to_synthesis(self):
...
class TestSafetyLimit:
    def test_exceeds_max_iterations_routes_to_end(self):
        ...
        cmd = node(base_state(iteration=10, max_iterations=10))

        assert cmd.goto == END
        assert cmd.update["errors"]
        assert "Safety limit" in cmd.update["errors"][0]["message"]

    def test_at_max_iterations_still_routes_to_end(self):
        ...
        cmd = node(base_state(iteration=5, max_iterations=5))

        assert cmd.goto == END


class TestAllBlocked:
    def test_all_blocked_routes_to_end(self):
...
class TestBlockedSkipped:
    def test_blocked_wp_is_skipped(self):
    ...
    def test_in_progress_processed_before_ready(self):
...
class TestRunLog:
    def test_routing_decision_logged_in_run_log(self):
    ...
    def test_state_iteration_incremented(self):
        """Supervisor must increment the iteration counter on every pass."""
        tools = make_mcp_tools(wp_list=[])
        node = make_supervisor_node(tools)
        cmd = node(base_state(iteration=3))
