...
| `extract_persona_model_slugs(workspace_root)` | `src.utils.persona_models` | Scans persona metadata and returns `{stage_id: model_slug}`. Resolution follows a four-layer priority chain: (1) `assignments.json` `persona_models[stage_id]` UUID resolved via `local.json` — skipped when resolved slug is `"inherit"`; (2) per-persona `model_slug` from the ledger YAML file; (3) `assignments.json` `default_model_uuid` resolved via `local.json` — skipped when resolved slug is `"inherit"`; (4) `default_model_slug` from `_shared.yaml`. Layers 1 and 3 are silently bypassed when `assignments.json` or `local.json` are absent. Used by `load_config()`. |
