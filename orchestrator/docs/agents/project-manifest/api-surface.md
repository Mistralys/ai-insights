...
| Symbol | Signature | Description |
|--------|-----------|-------------|
| `load_subagents` | `load_subagents(stage: str, workspace_root: Path \| str) -> list[dict[str, Any]]` | Returns a list of SubAgent spec dicts (`name`, `description`, `system_prompt` keys) for *stage*. Reads the `subagents` field from the ledger persona YAML for *stage* (via `find_ledger_yaml_for_stage()`), then resolves each slug by searching `personas/ledger-support/src/meta/{slug}.yaml` first (for `description`), falling back to `personas/standalone/src/meta/{slug}.yaml`. Similarly resolves `system_prompt` from `personas/ledger-support/deep-agents/{slug}.md` first, falling back to `personas/standalone/deep-agents/{slug}.md`. Returns `[]` for stages with no `subagents` key, unknown stage IDs, or when the workflow manifest is inaccessible. Raises `FileNotFoundError` if any declared slug has no matching YAML or deep-agents file in either suite. Raises `ValueError` if the resolved YAML lacks a `description` field. Results cached per `(stage, slug)` pair for the process lifetime — cache key excludes `workspace_root` (single-workspace assumption). |
