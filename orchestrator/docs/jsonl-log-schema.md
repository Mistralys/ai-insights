...
Every run writes a JSONL file to `orchestrator/logs/` during execution. At run completion it is moved to `mcp-server/storage/ledger/{slug}/` (path printed at run end). Each line is a JSON object. The schema supports **20 event types** across three emitters: the CLI (run lifecycle), the supervisor (routing and project progress), and stage nodes (pipeline execution).
