...
`restrict_to_wp(tools, wp_id)` is a **Layer 3 safety net** applied in WP-scoped stage nodes. For **write tools** it auto-injects the active `work_package_id` into any tool call that omits it, and raises `ValueError` on any tool call that explicitly passes a *different* `work_package_id`. This prevents a confused LLM from accidentally writing to the wrong work package. Passing an empty `wp_id` is a no-op (synthesis stages, which operate at project scope, are unaffected).
