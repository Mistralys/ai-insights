...
def _build_developer_prompt(state: "WorkflowState") -> str:
    ...
    return (
        ...
        f"2. Claim the work package by calling `ledger_claim_work_package`.\n"
        f"3. Start the implementation pipeline by calling "
        f"`ledger_start_pipeline` with `type='implementation'`.\n"
        f"4. Implement all required code changes to satisfy the acceptance "
        f"criteria listed in the work package.\n"
        f"5. Run any relevant tests to verify correctness.\n"
        f"6. Complete the pipeline by calling `ledger_complete_pipeline` with "
