...
- **Knowledge page** (`#/knowledge`) — browse and manage knowledge insights stored in the ledger's `.knowledge/` store; tab navigation switches between **Global** (cross-repository insights) and **Repository** (per-repository insights) scopes; all filtering is client-side (no round-trip on tab switch, category change, or search keystroke); scope badges (`.badge-scope-global` blue, `.badge-scope-repository` green) identify the insight origin at a glance; category pills (`.category-pill`) and tag chips (`.tag-chip`, purple tones) surface insight metadata; **confidence labels** (`.confidence-label`) show `formatConfidence()` output — e.g. `80% (High)` — using named bucket constants (`CONFIDENCE_HIGH_MIN = 68`, `CONFIDENCE_MEDIUM_MIN = 34`); card action rows (`.knowledge-actions`) expose per-insight operations: **Edit** (inline form that preserves the edit state on save failure — see error-handling note below), **Delete** (inline confirmation step), **Promote to Global** (repository-scoped cards only), and **Move to Repository** (repository slug input via `.knowledge-move-input`); all new classes are fully dark-mode covered
...
- **`API.getKnowledge(params)`** — `GET /api/knowledge[?scope=…&repository_name=…&…]`. Returns a parsed JSON response listing or searching knowledge insights. Builds the query string via `buildQueryString`; falsy/empty param values are silently omitted — pass `undefined` to leave a filter unset rather than sending `?key=undefined` to the server.

- **`API.updateKnowledge(id, scope, repositoryName, data)`** — `PATCH /api/knowledge/:id`. Merges `scope` and `repository_name` into the JSON body **after** the caller-supplied `data` object (`Object.assign({}, data, { scope, repository_name })`), so scope/name always take precedence over same-named keys in `data`. A falsy `repositoryName` (empty string, `null`, `undefined`) is coerced to `undefined` and omitted from JSON serialisation.

- **`API.deleteKnowledge(id, scope, repositoryName)`** — `DELETE /api/knowledge/:id?scope=…[&repository_name=…]`. Passes scope/name as URL query parameters so the server can locate the correct store file. A falsy `repositoryName` is omitted from the query string by `buildQueryString`.

- **`API.promoteKnowledge(id, scope, repositoryName)`** — `POST /api/knowledge/:id/promote?scope=…[&repository_name=…]`. **Sends no request body** — the server identifies the source insight via the query parameters alone. Returns the newly created global insight (which receives a new ID from the global store's `next_id` counter, different from the original repository insight ID).

- **`API.moveKnowledge(id, sourceScope, sourceRepositoryName, targetRepositoryName)`** — `POST /api/knowledge/:id/move`. Sends `source_scope`, `source_repository_name` (omitted when falsy — global-scope moves have no source repository name), and `target_repository_name` (always required, never coerced) in the JSON body. Returns the newly created insight in the target repository store (with a new ID from the target store).

> **Terminology note:** All layers — backend HTTP API, MCP tools, storage layer, and frontend (`gui/public/api-client.js`, `gui/public/views/knowledge.js`) — use `repository_name` consistently. The former `project_slug` / `projectSlug` identifiers have been fully retired.
