...
| `load_subagents(stage, workspace_root)` | `src.utils.subagents` | Returns a list of SubAgent spec dicts (`name`, `description`, `system_prompt`) for *stage*. Reads the `subagents` field from the ledger persona YAML for *stage*, then resolves each slug against `personas/standalone/src/meta/{slug}.yaml` (for `description`) and `personas/standalone/deep-agents/{slug}.md` (for `system_prompt`). `name` is the kebab-case slug itself. Returns `[]` for stages with no `subagents` key or unknown stage IDs. Raises `FileNotFoundError` for missing standalone YAML or deep-agents files. Raises `ValueError` for a standalone YAML missing `description`. Results cached per `(stage, slug)` for the process lifetime (`workspace_root` excluded from key — single-workspace assumption). |
