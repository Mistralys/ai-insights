...
| `insight_agent` / `role` coupling | Per-persona YAML `insight_agent` field (ledger: `personas/ledger/src/meta/N-*.yaml`; standalone: `personas/standalone/src/meta/*.yaml`) | `role` field in the same YAML (must be identical for ledger personas — validated by `scripts/build-personas.js` via `scripts/lib/insight-validation.js`); `personas/shared/partials/insight-capture.md` → `{{insight_agent}}` substitution; `personas/shared/partials/insight-compilation.md` → `{{insight_agent}}` filter; `insight_report_target` must be declared as a pair with `insight_agent`. Standalone personas without `role` are exempt from the identity check. |
